...
(function () {
  ...
  window.PERC_RELEASE_CHANNEL = RELEASE_CHANNEL;
    ...
    groups.forEach(function (grp, gi) {
      ...
      $("menuBody").querySelectorAll('.menu-item.sub[data-gi="' + gi + '"]').forEach(function (el) {
        ...
        el.onclick = function (ev) {
          ...
          if (ev.target.closest(".menu-fav") || ev.target.closest(".menu-hide")) return;   // 不响应来自星标的点击
...
//   返回最强层级 + 命中值 + parentLevel（命中层级下一级，供 folder 子匹配用）
  ...
  function processBuildingRows(rows) {
    ...
    var iName = findCol(["name", "名称", "设备名称", "设施名称", "建筑物名称", "项目名称", "点位名称", "感知设备名称"]);
    if (iName < 0) {
  ...
  function showHelp() {
    ...
    $("genBody").innerHTML =
      ...
      "<p style='font-size:13px;color:#555;margin-bottom:6px'>水利感知项目一张图 · 四端（Android / iOS / Win11 / 统信 UOS）功能一致，以下为常用功能指引。</p>" +

      "<p><b>① 筛选</b>：按管理处 / 管理所 / 管理站 / 设备类型组合过滤，或顶部搜索框关键字检索；筛选默认值持久化（重启即套用）。</p>" +

      "<p><b>② 添加</b>：菜单「添加感知设施设备」→ 点地图放置 → 填写信息、所属系统、设备型号、固定资产编号、主要参数、上传照片。</p>" +

      "<p><b>③ 修改 / ④ 删除</b>：点击标记弹出 → 「修改」编辑，或「删除」。</p>" +

      "<p><b>⑤ 导航</b>：弹窗点「导航」，唤起手机已装地图 App。</p>" +

      "<p><b>⑥ 测距 / ⑦ 周边搜索</b>：菜单「测距」依次点地图取各段距离；「周边搜索」选中心与半径查范围内设施，支持当前位置 / 地图选坐标 / 按类型筛选。</p>" +

      "<p><b>⑧ 获取坐标</b>：菜单「获取坐标」支持 GPS 定位 / 点选地图 / 搜索设施三种方式，可一键复制。</p>" +

      "<p><b>⑨ 快捷常用（收藏）</b>：菜单项右侧 ☆ 点选收藏；子菜单内<b>长按 600ms</b>也可添加 / 移除「快捷常用」（与 ☆ 并存，解决手机/平板星标点不准、易误触），长按带振动与 toast 反馈。</p>" +

      "<p><b>⑩ 智能AI</b>：菜单「智能AI」可基于本地监测设施记录问答 / 补全 / 存疑标注（内部资料「本地辅助」不联网编造）；「设置 → 智能AI设置」可自定义大模型接口、多模型与自动调用策略。查询结果支持单击复制关键词 / 双击自动复制。</p>" +
      ...
      "<p><b>⑭ 导入 / 导出</b>：ovkmz / obj / CSV / XLSX / 照片批量导入；导出照片 / 设施表格（字段列可勾选、文件名可自定义）、ovkmz / obj（导出前对话框）。</p>" +
      ...
      "<p><b>⑱ 知识库入库与升级备份</b>：外部文档 pdf/docx/xlsx/csv/txt/md/网页 智能转 Markdown 入库（OCR 识别，全过程进度条）；升级数据导出可自定义文件夹与文件名（默认 感知设备一张图_YYYYMMDD.bak），导入前提示确认最新备份并覆盖数据。</p>" +
      ...
      (RELEASE_CHANNEL === "public"
        ? "<p style='color:#b8860b;margin-top:8px'>📢 当前为<b>公开测试版</b>：内置为演示数据，不含单位内部感知设备信息；导入/导出、智能AI 等均基于演示数据，便于功能验证与对外分发测试。</p>"
  ...
  function showAbout() {
    ...
    $("genBody").innerHTML =
      ...
      (RELEASE_CHANNEL === "public"
        ? "<p style='color:#b8860b'>📢 本程序为<b>公开测试版</b>：内置数据为演示示例，<b>不含任何单位内部感知设备信息</b>；AI 仅基于演示数据作答，请自行在「设置 → 智能AI设置」填写密钥。</p>"
  ...
  var CHANGES = [
    ...
    { v: "v1.44", d: "2026-09-10", items: [
      "优化（口令提示）：内部版所有对话框/提示语不再出现具体口令数字，仅提示「默认口令即开发者分机号」。",
      "优化（防误触）：菜单子项与右侧「隐藏 / 收藏」按钮间距拉大，点击区同步放大。",
      "修复（导入设备 CSV 报「未找到名称列」）：编码自动识别（UTF-8 / GB18030）+ 分隔符自动识别 + 名称列智能匹配；兼容奥维 Comment 列与本应用 comment 列（| / ; 分隔）及宽表属性列。",
      "优化（导出设备 CSV）：参数列头改 comment，条目的 ; 分隔改为 |。",
      "优化（ovkmz 导出）：备注改为「键 : 值」以 | + 换行分隔。",
      "新增（本地向量智能化内核）：参数反查设备、类型与单位统计、预案文档切片关联、报告导出 doc/docx + 预览打印、PDF 转 Word、设备类型释义向量库。",
      "新增（图片预览交互）：滚轮/拖拽、双指缩放、长按菜单。",
      "新增（运维记录导出）：支持导出 Word（doc / docx）与 PDF（预览打印另存）。"
    ],
    },


    { v: "v1.42", d: "2026-09-09", items: [
    ...
    { v: "1.34", d: "2026-09-05", items: [
      "修复安卓端升级菜单「下载更新包」点击报错：改由系统浏览器/网盘App接管打开"
    ]},

    { v: "v1.32", d: "2026-09-05", items: [
      "内部版（含完整单位内部感知设备数据，仅限单位内部使用）：与水利 v3.56 / 古建 v3.7.3 同步。偶数版保留真实感知设备坐标/子系统/设备编码，程序标注「内部版」。知识库智能化/OCR/升级备份/四端对照单与 v1.31 相同。"
    ]},

    { v: "v1.31", d: "2026-09-05", items: [
      "知识库智能化与三端四平台同步（与水利 v3.55 / 古建 v3.7.3 同步，感知核心=感知监测设备，保留 subsystem/equip_type 元数据，功能不删不减）：①知识库切片保持语句相对完整——以「整句」为最小粒度递归打包，重叠按整句回溯，切片绝不在句中截断；②知识库保存前先切片再落盘——kbAdd/导入/OCR入库/Hermes沉淀/查询保存统一经 kbSave 写入口（chunks+签名缓存，正文变更才重切），向量索引优先复用已存切片；③RAG 智能检索四端同步：向量余弦 0.45 + BM25 0.35 + 模糊 0.20 混合检索、内容反查条目、模糊/智能查询、智能生成提示词、结合大模型智能问答（引用标注，失败降级抽取式答案）、记忆管理 + Hermes 自我学习；④轻量化 OCR 读取外部文档(pdf/docx/xlsx/csv/txt/md)智能转 md 入库同步四端；⑤升级数据导出自定义文件夹/文件名（默认 感知设备一张图_YYYYMMDD.bak），导入增加「请确认这是最新备份，导入会覆盖程序中的全部数据」预检与合并/覆盖双模式，导出→导入闭环实测通过；⑥内部版（偶数通道）支持百度网盘自动升级；⑦写备忘录「运行错误:script error」修复（宿主数据数组 getter 桥接）；⑧信息与帮助→功能介绍/版本变更/四端功能对照单全面更新至 v1.31。Android / Win11 MSI+EXE / 统信 UOS deb / iOS PWA 四平台全量同步，改动前已备份可回退。"
    ]},
    { v: "v1.29", d: "2026-09-04", items: [
      "与水利 v3.53 同步——修复升级/备忘录菜单脚本错误（闭包函数全局桥接）；知识库 md/txt/html 导入导出与外部文件(pdf/docx/xlsx/csv)智能转 Markdown 入库（带进度条）。"
    ]},
    { v: "v1.27", d: "2026-09-04", items: [
      "公开测试版（与水利 v3.51 / 古建 v3.7.1 同步）：运行维护菜单新增「写备忘录 / 我的备忘录」（所见即所得编辑器，默认绑定感知设备，支持关键词筛选与智能AI 查询）。"
    ]},
    { v: "v1.26", d: "2026-09-04", items: [
      "内部版（含完整单位内部感知设备数据，仅限单位内部使用）：偶数版保留真实感知设备坐标/子系统/设备编码，程序标注「内部版」。"
    ]},

    { v: "v1.25", d: "2026-09-02", items: [
      "文档与诊断增强（与水利 v3.49 同步，感知核心=感知切切实设备，保留 subsystem 子系统 + equip_type 设备类型元数据）：①菜单「帮助」更名为「功能介绍」，消除与「关于」歧义，并补全新增功能说明（快捷常用长按、智能AI、知识库、运行维护、设置-天地图密钥/管理所、错误日志排查、统计、导出自定义文件名等）；②信息与帮助→版本变更、四端功能对照单增补至最新；③新增全平台启动诊断：app 启动即在控制台与本地诊断缓冲记录当前版本/构建日期/运行平台（UOS 另由 server.py 写入 launch_err.log，版本号随版本自动更新）；④UOS 保留 no-store 防缓存头与每次启动写 launch_err.log + install.log。"
    ]},
    { v: "v1.24", d: "2026-09-01", items: [
  ...
  var PLATFORM_COMPARE = [
    ...
    { v: "v1.40", d: "2026-09-08", note: "本版（内部版，与水利 v3.64 / 古建 v3.7.7 同步）：①新增「智能化内核」菜单组（智能模糊检索 / 提示词生成器 / AI 记忆·Hermes / 存疑反向查询 / 强制联网）；②内部版加启动口令并把内部 PWA 数据整体加密后部署到公开仓库，解决私有仓库 Pages 不可用的同时防泄密；③GitHub 升级内部渠道免登录；④安卓 ovkmz 导入自动定位修复；⑤菜单 script error 根治。", rows: [
    ...
    { v: "1.31", d: "2026-09-05", note: "本版（知识库智能化与四平台同步）：①知识库切片保持语句相对完整（整句打包）；②知识库保存前先切片再落盘（统一写入口 + 签名缓存）；③RAG 智能检索/反向查询/智能问答/记忆+Hermes 四端同步；④OCR 外部文档入库四端同步；⑤升级备份导出自定义目录/文件名 + 导入预检（最新备份警示/合并-覆盖）；⑥内部版百度网盘自动升级；⑦备忘录 script error 修复。", rows: [
      { f: "知识库智能切片（语句完整，保存前先切片）", a: "✅", i: "✅", w: "✅", u: "✅", n: "1.31 整句打包+签名缓存" },
      ...
      { f: "升级备份导出自定义目录/文件名（默认 感知设备一张图_YYYYMMDD.bak）", a: "✅ 原生桥", i: "✅ 浏览器下载", w: "✅ 浏览器下载", u: "✅ 浏览器下载", n: "导入预检：最新备份警示 + 合并/覆盖" },
      { f: "写备忘录 / 我的备忘录（运行维护）", a: "✅", i: "✅", w: "✅", u: "✅", n: "v1.27 新增，v1.29 script error 已修复" },
      { f: "每页退出按钮 + 三击空白呼出主菜单", a: "✅", i: "✅", w: "✅", u: "✅", n: "全平台一致" }
    ]},

    { v: "v1.25", d: "2026-09-02", note: "本版（文档与诊断增强，与水利 v3.49 同步）：①菜单「帮助」更名为「功能介绍」；②信息与帮助→版本变更、四端功能对照单增补至最新；③新增全平台启动诊断（app 启动即记录版本/构建日期/运行平台，UOS 另写 launch_err.log）；④UOS 保留 no-store 防缓存 + 每次启动写 launch_err.log/install.log；保留感知核心（subsystem 子系统 + equip_type 设备类型元数据）；四平台同步保持。", rows: [

      { f: "功能介绍（原「帮助」改名，新增功能指引）", a: "✅", i: "✅", w: "✅", u: "✅", n: "v1.25 消除与「关于」歧义" },

      { f: "全平台启动诊断（版本/平台随版本记录）", a: "✅ 控制台+本地", i: "✅ 控制台+本地", w: "✅ 控制台+本地", u: "✅ 控制台+本地", n: "v1.25 app 启动即记录" },

      { f: "UOS no-store 防缓存 + 每次启动写 launch_err.log/install.log", a: "—", i: "—", w: "—", u: "✅ 浏览器壳", n: "v1.24 起" }

    ]},

    { v: "v1.22–v1.24", d: "2026-08-29 → 2026-09-01", note: "期间跨版本重点（与水利同步）：智能AI×知识库深度融合（本地优先 🔒 → 在线 🌐）、建筑骨干知识库随包内置（1125 条）；设置新增天地图密钥（三层回退）；筛选管理处可多选 + 导出管理处列；ZIP 最长匹配；子菜单长按 600ms 添加/移除快捷常用；UOS deb 纯浏览器壳 + control 四件套 + 独立端口 7206；Win WebView2 userDataFolder 用户可写根治 E_ACCESSDENIED。", rows: [

      { f: "智能AI×知识库深度融合（本地优先→在线兜底）", a: "✅", i: "✅", w: "✅", u: "✅", n: "v1.22 本地🔒/在线🌐" },

      { f: "建筑骨干知识库随包内置（1125 条）", a: "✅", i: "✅", w: "✅", u: "✅", n: "v1.19 起内置" },

      { f: "设置→天地图密钥（三层回退，保存即重建底图）", a: "✅", i: "✅", w: "✅", u: "✅", n: "v1.21 加" },

      { f: "筛选→管理处可多选 + 导出管理处列", a: "✅", i: "✅", w: "✅", u: "✅", n: "v1.21" },

      { f: "ZIP 三级匹配升级为最长匹配", a: "✅", i: "✅", w: "✅", u: "✅", n: "v1.21" },

      { f: "子菜单长按 600ms 添加/移除快捷常用（与 ☆ 并存）", a: "✅", i: "✅", w: "✅", u: "✅", n: "v1.24" },

      { f: "UOS deb 纯浏览器壳 + control 四件套 + 独立端口 7206", a: "—", i: "—", w: "—", u: "✅ mips64el/loongarch64/arm64/amd64", n: "v1.24" }
  ...
  function initAIModule() {
    ...
    window.AIModule.init({
      ...
      // v3.43+v3.45+v3.46：按核心实体差异化；感知核心=「感知切切实设备」；domain 必须感知化
      ...
      // v3.43+v3.45+v3.46：感知设备关心「子系统」字段（视频监控/雨水情/大坝安全/地下水源四类子系统）

      fieldSchema: ["office","station","btype","chan","subsystem"],
      ...
      searchRecords: function (q) {
        ...
        return BUILDINGS.filter(function (b) { return !q || ["name","office","station","btype","subsystem"].some(function (f) { return (b[f] || "").indexOf(q) >= 0; }); });
      ...
      applyUpdate: function (rec, patch) {
        ...
        ["office","station","btype","chan","subsystem"].forEach(function (k) { if (patch[k] != null && String(patch[k]).trim() !== "") rec[k] = patch[k]; });
...
/* 水利感知平台：备忘录模块配置（v1.27） */
nmInstall({ kind: "备忘录", docNoun: "感知设备", appName: "水利感知平台", lsKey: "perc_memos_v1", fileTag: "perc", bindLabel: "绑定感知设备", recs: function () { return BUILDINGS; }, recName: function (r) { return (r.name || "?") + (r.office ? ("（" + r.office + "）") : ""); } });
...
upInstall({
  ...
  exportBase: "感知设备一张图",
  exportDir: "感知设备一张图备份",
  manifestUrl: "https://020271252c9a4b9fab19848e25a3f6e3.app.workbuddy.link/manifests/perc.json"
});
 ...
 * 感知奇偶双通道：公开版(奇数)→ganzhi-yitu-5090pub；内部版(偶数)→ganzhi-yitu5090。 */
var upGithubCfg = { public: "g101400/ganzhi-yitu-5090pub", internal: "g101400/ganzhi-yitu5090-sec" }
var upPwaCfg = { public: "https://g101400.github.io/ganzhi-yitu-5090pub/", internal: "https://g101400.github.io/ganzhi-yitu5090-sec/" };;
function ghChannel() {
